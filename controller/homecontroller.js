...
module.exports.home = function(req,res){
    ...
    //     })
    // })
    TodoLists.find({}).then(function(todo){
        return res.render('homePage',{
            title:"home",
            todoList:todo
        })
...
module.exports.deleteTodo = function(req,res){ 
    ...
    for(let i=0;i<newsp.length;i++){ // looping over newsp  to delete all the checked value
        TodoLists.findByIdAndDelete(newsp[i]).catch(function(err){
            console.log(err);
        })
    }
return res.redirect('/');
...
module.exports.EditPage = function(req,res){ // here we are fetching the data whic need to be edited
    ...
    TodoLists.findById(req.query.id).then(function(todoLists){
...
module.exports.editDetails = function(req,res){
    ...
    let newdate='';
    newdate= DateValeu(dueDate);     
     TodoLists.updateOne({_id:req.query.id},{$set:{desc:req.body.desc,category:req.body.category,dueDate:newdate}} ).then(function(err,todoData){
        return res.redirect('/')
    }).catch(function(err){
        console.log(err)
    })
